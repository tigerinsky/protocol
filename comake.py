...
def _generate_pb_files(proto):
    ...
    (ret, err, out) = _execute('%s -I=%s --cpp_out=%s %s' % (g_protoc, g_cur_dir, g_cur_dir, proto))
    print '%s -I=%s --cpp_out=%s %s' % (g_protoc, g_cur_dir, proto_dir, proto)
    if 0 != ret:
...
def APP(bin_name, target_files):
    ...
    for target in target_files:
        if target.endswith('.proto'):
            ...
            g_cpps.add(target)
    g_apps[bin_name] = target_files

def STATIC_LIB(lib_name, target_files, header_files):
    global g_libs
    global g_cpps
    global g_protos
    global g_thrifts
    ...
    for target in target_files:
        if target.endswith('.proto'):
            prefix = target[0 : target.rfind('.')]
            g_protos.add(target)
            header_files.append(prefix + '.pb.h')
            targets.append(prefix + '.pb.cc')
        elif target.endswith('.thrift'):
            pass
        else:
            g_cpps.add(target)
